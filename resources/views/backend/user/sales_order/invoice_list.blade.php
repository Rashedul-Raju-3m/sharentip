	...
	<div class="col-lg-12">
		<div class="card no-export">
		    <div class="card-header d-flex align-items-center">
				<span class="panel-title">{{ _lang('Invoice') }}</span>
...
{{--										<a href="{{ route('invoice_create_against_sales_order', $salesOrder->id) }}" data-title="{{ _lang('Create Invoice') }}" class="dropdown-item dropdown-edit"><i class="ti-pencil mr-1"></i> {{ _lang('Create Invoice') }}</a>--}}
										<a href="{{ route('invoice_edit_agst_so', $salesOrder->id) }}" data-title="{{ _lang('Update Sale Order') }}" class="dropdown-item dropdown-edit"><i class="ti-pencil mr-1"></i> {{ _lang('Edit') }}</a>
