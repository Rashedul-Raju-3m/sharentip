                        ...
                        <div class="card-body">
                            ...
                            <table class="table table-striped table-bordered" id="customers_table">
                                ...
                                <tbody class="Entries-body">
                                    @if($newEntriesItems && count($newEntriesItems)>0)
                                        @foreach($newEntriesItems as $key => $item)
                                            <tr>
                                                <td class="text-center">
                                                    <select class="form-control auto-select select2" data-selected="{{ old('parent_group') }}" name="parent_id{{$key}}" id="ledger_id{{$key}}">
                                                        <option value="">{{ _lang('Select Ledger') }}</option>
                                                        @if($data['ledgers'] && count($data['ledgers'])>0)
                                                            @foreach($data['ledgers'] as $key => $ledger)
                                                                <option value="{{$ledger['id']}}">{{ $ledger['name'] }}</option>
