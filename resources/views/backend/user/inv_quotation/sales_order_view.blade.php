        ...
        <div class="col-lg-10 offset-lg-1">
            <div class="card">
                <div class="card-header d-sm-flex align-items-center justify-content-between">
                    <span class="panel-title">{{ _lang('Sales Order View') }}</span>
...
{{--                        <h4>{{ $entryType?$entryType->name:'' }}</h4>--}}
                        <h5>{{ _lang('Entries Not Approve') }}</h5>
...
{{--                        <h5>Date : {{ date('d-m-Y',strtotime($entries->date)) }}</h5>--}}
                    ...
                    {{--<div class="card-footer d-sm-flex align-items-center justify-content-between">
                        <a class="btn btn-outline-primary btn-xs" href="{{route('entries_approve',$entries->uu_id)}}"><i class="fas fa-print mr-1"></i>{{ _lang('Approve') }}</a>
                        <a class="btn btn-outline-primary btn-xs" href="{{route('entries_edit',$entries->uu_id)}}"><i class="fas fa-print mr-1"></i>{{ _lang('Edit') }}</a>
                    </div>--}}
                </div>
            </div>
        </div>
