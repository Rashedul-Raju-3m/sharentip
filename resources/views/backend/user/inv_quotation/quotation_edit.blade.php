...
<div class="row">
	<div class="col-lg-12">
		<div class="card">
			...
			<div class="card-body">
			    <form method="post" class="validate" autocomplete="off" action="{{ route('store_quotation') }}" enctype="multipart/form-data">
					{{ csrf_field() }}
					<div class="row">
                        <input type="hidden" name="quotation_id" value="{{$salesOrder->id}}" id="sales_order_id">
                        ...
                        <a data-href="{{route('get_quantity_wise_product_body_quotation')}}" id="get_quantity_wise_product_body_quotation"></a>
                                    ...
                                    @foreach($items as $item)
                                        <tr>
                                            ...
                                            <td class="text-center" style="width: 15%">
                                                <input type="text" name="rate" value="{{$item->unit_cost}}" id="unit_cost" data-id="{{$item->id}}" class="form-control text-right">
                                            </td>
...
@section('js-script')
    <script type="text/javascript">
        ...
        $(document).delegate('#quantity,#unit_cost','blur',function () {
            let quantity = $('#quantity').val();
            let unit_cost = $('#unit_cost').val();
            let itemID = $(this).attr('data-id');
            console.log(quantity,unit_cost,itemID)
            /*let route = $('#get_quantity_wise_product_body_quotation').attr('data-href');

            $.ajax({
                url: route,
                method: "GET",
                dataType: "json",
                data: {itemID: itemID,quantity:quantity,unit_cost:unit_cost},
                beforeSend: function( xhr ) {
