...
//        dd($orders);
    ...
    public function userBillingInfo(){
        ...
        return view('frontend.user_billing_info',compact('user','languages','country','divisions','districts','upazilas'));
    }
    ...
    public function serviceCategoryDetails($id){
        $serviceCategories = ServiceCategory::where([['is_active',1],['is_delete',0]])->orderBy('id','ASC')->get();
        $serviceCategory = ServiceCategory::find($id);
        $service = Service::where([['service_category_id',$id],['is_active',1],['is_delete',0]])->get();

        return view('frontend.service.index',compact('serviceCategories','serviceCategory','service'));

    }

    public function serviceDetails(){
        $serviceID = $_GET['serviceID'];
        $service = Service::where([['id',$serviceID],['is_active',1],['is_delete',0]])->first();
        $serviceCategory = ServiceCategory::find($service['service_category_id']);

        $response = [];
        if($service && $serviceCategory) {
            $body = \Illuminate\Support\Facades\View::make('frontend.service._service_details',compact('service','serviceCategory'));
            $contents = $body->render();
            $response['content'] = $contents;
            $response['result'] = 'success';
            $response['category_name'] = 'Service details ( '.$serviceCategory->name.' )';
        }else{
...
//        return $serviceID;
    ...
    public function cmsPage($slug){
        return view('frontend.cms_page',compact('slug'));
    }
    ...
    public function eventServiceSearch(Request $request){
        $input = $request->all();
        if (isset($input['search'])){
            $events = Event::where(['events.is_deleted'=>0,'events.status'=>1])
                            ->select([
                                'events.id',
                                ...
                                'events.start_time',
                                'events.end_time',
                                'category.name as event_category_name',
                                'users.name as organizer_name'
                            ])
                            ->leftjoin('category','category.id','=','events.category_id')
                            ->leftjoin('users','users.id','=','events.user_id')
                            ->where('events.name','LIKE','%'.$input['search'].'%')
                            ->orWhere('category.name','LIKE','%'.$input['search'].'%')
                            ->orWhere('events.type','LIKE','%'.$input['search'].'%')
                            ->orWhere('events.instruction','LIKE','%'.$input['search'].'%')
                            ->orWhere('events.address','LIKE','%'.$input['search'].'%')
                            ->orWhere('events.description','LIKE','%'.$input['search'].'%')
                            ->orWhere('events.tags','LIKE','%'.$input['search'].'%')
                            ->get()->toArray();
        }

        $services = Service::where(['service.is_delete'=>0,'service.is_active'=>1])
                            ->select([
                                'service.id',
                                ...
                                'service.contact_person_mobile',
                                'service.service_type',
                                'service_category.name as service_category_name',
                            ])
                            ->leftjoin('service_category','service_category.id','=','service.service_category_id')
                            ->where('service.name','LIKE','%'.$input['search'].'%')
                            ->orWhere('service_category.name','LIKE','%'.$input['search'].'%')
                            ->orWhere('service.company_name','LIKE','%'.$input['search'].'%')
                            ->orWhere('service.contact_person_name','LIKE','%'.$input['search'].'%')
                            ->orWhere('service.contact_person_mobile','LIKE','%'.$input['search'].'%')
                            ->orWhere('service.details','LIKE','%'.$input['search'].'%')
                            ->get()->toArray();
//        dd($services);
        return view('frontend.event_service_search',compact('events','services','input'));
    }
