    ...
    public function tax_report(Request $request) {
        if ($request->isMethod('get')) {
            ...
            if ($report_type == 'paid_unpaid') {
                ...
                AND invoices.invoice_date >= '$date1' AND invoices.invoice_date <= '$date2' RIGHT JOIN taxes ON taxes.id=invoice_item_taxes.tax_id AND taxes.business_id = $business_id GROUP BY taxes.id");
                ...
                RIGHT JOIN taxes ON taxes.id=purchase_item_taxes.tax_id AND taxes.business_id = $business_id GROUP BY taxes.id");
            }
            ...
            if ($report_type == 'paid') {
                ...
                AND invoice_items.business_id = $business_id JOIN invoices ON invoices.id=invoice_items.invoice_id AND invoices.status != 0 AND invoices.is_recurring = 0
                AND invoices.paid > 0 AND invoices.invoice_date >= '$date1' AND invoices.invoice_date <= '$date2' RIGHT JOIN taxes ON taxes.id=invoice_item_taxes.tax_id AND taxes.business_id = $business_id GROUP BY taxes.id");
